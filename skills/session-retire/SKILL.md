...
⑦ 服务端归档（v2.18 新增，需 0.24+ 引擎）：
   ...
   PORT=$(python -c "import json,os,glob; f=sorted(glob.glob(os.path.expanduser('~/.kimi-code/server/instances/*.json'))); print(json.load(open(f[0]))['port'] if f else 5494)" 2>/dev/null || echo 5494)
