...
| Skill | 级别 | 效果 |
|-------|:--:|------|
| `kimi-session-orchestrator` | Agent | 加载时触发 PM 启动协议：读取 coordinator-guide → 询问角色/目标/决策模式。后续操作自动遵循即发即返、后台轮询、红线规则，防止 MCP 超时和 token 浪费 |
...
| 场景 | 涉及 Skill |
|------|-----------|
| 新 session 首次使用 kimi-session-orchestrator | `kimi-session-orchestrator` 自动触发启动协议 |
| 批量创建 task session 执行并行任务 | `kimi-session-orchestrator` 自动约束即发即返 + 后台轮询 |
...
新 session 加载 `kimi-session-orchestrator` skill 后自动触发 PM 启动协议——读取 coordinator-guide 建立基线，询问角色、目标和决策模式。`/reload` 后生效。
...
| 日期 | 版本 | 变更 |
|------|:--:|------|
| 2026-07-11 | v2.8 | 修复注入文本歧义；README 重构：TOC + Skill 说明（4 个 skill 效果与使用场景表）+ 环境变量表 + FAQ + 贡献指南 + 版本历史；端口全部可配置化；全文档过时清理 |
