...
```bash
# 终端 1: Kimi Server（Tunnel 自动检测端口，无需 --port）
kimi web --no-open
# 记下输出的 Token
...
> **注意**：`args` 中的路径需为绝对路径，按实际 clone 位置调整。Tunnel 自动从 lock 文件检测端口，无需设置 `KIMI_SERVER_URL`。`/reload` 后生效。
...
| Kimi Server 端口 | ✅ | 自动从 `~/.kimi-code/server/lock` 检测，跨平台无需手动配置 `--port`（v2.8+） |
