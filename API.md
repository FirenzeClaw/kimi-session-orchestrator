...
> **0.29.0 变更**：Server 不再写入 `~/.kimi-code/server/lock`，仅写入 `server/instances/<id>.json`（新增 `server_id`、`heartbeat_at`、`host_version` 字段；`started_at` 从 string 改为 epoch ms number）。`server-lock.ts` 已适配双格式回退（legacy lock → instances/ → fallback）。**v2.24 起只读化**：不再做 PID 活性/`heartbeat_at` 30s 心跳 stale 检测与文件清理——实例文件仅作端口线索，陈旧文件由启动时自动激活流程（`server-spawner.ts`）覆盖。
